...
bot.on('message', message => {
  //function that converts message format into api-readable match codes.
  ...
  //function that coverts message format into api-readable event codes.
  function makeTBAEventCode(eventInput1, eventInput2) {
      if(isNaN(eventInput2)) {
        var eventCode = eventInput1 + eventInput2.toLowerCase();
      } else if (isNaN(eventInput1)) {
        var eventCode = eventInput2 + eventInput1.toLowerCase();
      } else {
        var eventCode = 'error';
      }
    return eventCode;
 ...
 (message.content.match(/^TBA EVENT ([0-9]+) ([A-Z]+)|^TBA EVENT ([A-Z]+) ([0-9]+)/gi) || [] ).forEach(function(match) {
   var failedEventCheck = false;
   var eventSplit = message.content.split(/\s+/g);
   var eventCode = makeTBAEventCode(eventSplit[2],eventSplit[3]);
   //Convert event code to information to list out
   tba.getEventViaCode(eventCode, function(err,event_object) {
     if (!err) {
       try {
         event_object['key'];
       } catch (e) {
         failedEventCheck = true;
       }
       if (!failedEventCheck) {
            if (event_object['year'] == 2017) {
              if(event_object['official']==true) {
                message.channel.sendMessage(':green_book: This is an offical FIRST competition that will happen in week ' + event_object['week'] + ' (starts ' + event_object['start_date'] + ')');
              } else {
                message.channel.sendMessage(':red_circle: This is an unoffical (offseason) competition that started on ' + event_object['start_date']);
              }
            } else {
              if(event_object['official']==true) {
                message.channel.sendMessage(':green_book: This was an offical FIRST competition that happened on week ' + event_object['week'] + ' (started ' + event_object['start_date'] + ')');
              } else {
                message.channel.sendMessage(':red_circle: This was an unoffical (offseason) competition that started on ' + event_object['start_date']);
              }
            }
            message.channel.sendMessage( 'Event type: ' + event_object['event_type_string'] +  '\n' + 'Name: ' + event_object['name'] + '\n' + 'Location: ' + event_object['location'] + '\n' + 'View on TBA http://www.thebluealliance.com/event/' + eventCode);
       } else if(failedEventCheck==true) {
            message.channel.sendMessage('Failed to find event with the code: ' + eventCode + '. Need help? try `TBA HELP`');
       }
     }
   });
 });
